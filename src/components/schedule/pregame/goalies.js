...
useEffect(() => {
    ...
    fetch(
      `https://statsapi.web.nhl.com/api/v1/people/${player.person.id}/stats?stats=statsSingleSeason&season=20222023&limit=2`,
      {
        mode: "cors",
      }
    )
      .then((response) => response.json())
      .then((response) => setTendies(tendies => [...tendies,{person: player.person, position: player.position, jerseyNumber: player.jerseyNumber, stats: response.stats[0].splits} ]))
...
if (tendies === undefined) {
    ...
    return (
        <table id="tendy-table">
            ...
            <tbody>
                {tendies.filter(player => (player.position.abbreviation === "G") && (player.stats[0] !== undefined)).map((goalie) => {
                    return (
                        <>
