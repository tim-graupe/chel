    ...
    const awayLastTen = () => {
      fetch(
        ...
        .then((response) =>
          setTeamResults(
            response.records
              .filter((team) => team.division.id === props.team.division.id)[0]
              .teamRecords.filter((away) => away.team.id === props.team.id)[0]
          )
        )
    ...
    return (
      <table className="last-ten-table">
        ...
        <tbody>
          {teamTen.map((game) => {
            return (
              <tr key={game.games[0].gamePk}>
                <td>
                  <img
                    src={`https://www-league.nhlstatic.com/images/logos/teams-current-primary-light/${game.games[0].teams.away.team.id}.svg`}
                    alt="team-logo"
                    className="last-ten-logos"
                  />{" "}
                  {game.games[0].teams.away.score}{" "}
                  {game.games[0].teams.home.score}{" "}
                  <img
                    src={`https://www-league.nhlstatic.com/images/logos/teams-current-primary-light/${game.games[0].teams.home.team.id}.svg`}
                    alt="team-logo"
                    className="last-ten-logos"
                  />{" "}
                </td>
